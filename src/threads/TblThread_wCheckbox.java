...
public class TblThread_wCheckbox extends Thread{
 ...
 private boolean continuar = true;
  ...
  public TblThread_wCheckbox(String sql,JTable tabla,int[] tipo_de_cst,String[] parametros,Connection con,JInternalFrame p,int booleanColumn){
       this.sql=sql;       
       ...
       this.con=con;
       this.p=p;
       this.booleanColumn=booleanColumn;//IMPORTANTE, COMIENZA DESDE CERO (0)
   }
            ...
            if(rs.isBeforeFirst()){
             while(rs.next()){
                for (int i = 0; i < datos.length; i++){                     
                     if(i==booleanColumn){
                        datos[booleanColumn]=rs.getBoolean(i+1);
                        if(datos[booleanColumn]==null)datos[booleanColumn]=Boolean.FALSE;
                    }else{
